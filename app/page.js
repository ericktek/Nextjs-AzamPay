...
const CheckoutPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    // Validate fields
    if (!accountNumber || !amount || !externalId) {
      setError('All fields except currency and provider are required.');
      return;
    }
  ...
  return (
    <div className="max-w-md mx-auto mt-10 p-6 border border-gray-300 rounded-lg shadow-lg">
      {token ? (
        <div>
          <h1 className="text-gray-700 mb-4 font-semibold">AZAMPAY MNO CHECKOUT</h1>

          <form onSubmit={handleSubmit}>
            <div className="mb-4">
              <label htmlFor="accountNumber" className="block text-gray-700 mb-2">Phone Number:</label>
              <input
                id="accountNumber"
                type="text"
                value={accountNumber}
